...
One solution is a "single-class-per-file" approach. Keep one class per file. This also improves Git usability. Developers can work on separate files, which means fewer merge conflicts. It makes code easier to find too. When each file has one purpose, understanding a component takes less effort. It is also easier to prompt AI models because you do not need to copy the whole codebase into the context.
