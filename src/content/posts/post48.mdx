...
These optimizations are also part of Cactus Compute's compatibility claims. The vendor reports over 500 tokens per second decode speed on a Raspberry Pi 5, 300 to 1500 tokens per second on listed headsets and phones depending on hardware, and support for devices such as the ESP32-S3 at the reported memory budget [1].
