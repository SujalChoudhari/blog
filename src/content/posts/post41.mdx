...
Sequence diagrams are a useful check for race conditions. On one occasion, a PR added an auto-reconnect feature to bxios. The written summary claimed everything was handled, but the sequence diagram showed a flaw: incoming WebSocket frames received during a socket reconnect attempt were dropped because the frame buffer queue was initialized after the connection opened, not before. The diagram exposed a timing issue that the unit tests had missed. It did not prove that the rest of the implementation was correct.
